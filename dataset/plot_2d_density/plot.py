...
def stats(example):
    sum_toks, doc_toks = tokenization_stats(
        example["summary"], example["document"], tokenizer
    )
...
sns_plot = sns.jointplot(x=x, y=y, kind="kde").set_axis_labels(
    "# of toks in summary", "# of toks in document"
)
sns_plot.savefig("dataset/plot_2d_density/2d_density.png")

sns_plot = sns.jointplot(
    x=x, y=y, kind="kde", xlim=(0, 2000), ylim=(0, 10000)
).set_axis_labels("# of toks in summary", "# of toks in document")
sns_plot.savefig("dataset/plot_2d_density/2d_density_zoom.png")

sns_plot = sns.jointplot(
    x=x, y=y, kind="kde", xlim=(0, 800), ylim=(0, 4000)
).set_axis_labels("# of toks in summary", "# of toks in document")
