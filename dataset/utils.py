...
async def index_sources(wikinews_json_path, index_path, html_path, json_path, max_url_count, max_workers, restart):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:        
        ...
        tasks = [
            loop.run_in_executor(
                executor,
                index,
                *(url, index_path, index_template, html_template, json_template) # Allows us to pass in multiple arguments to `fetch`
            )
            for url in sources
        ]
